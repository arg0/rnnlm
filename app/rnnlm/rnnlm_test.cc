...
#include "app/rnn/layers/scrn_layer.h"
...
bool AreGradientsCorrect(
    Matrix& params,
    std::tr1::function<Real (const Matrix&)> calc_cost,
    std::tr1::function<Matrix (const Matrix&)> calc_grads) {
...
bool IsDerivativeCorrect(Real param, std::tr1::function<Real (Real)> calc_cost,
                         std::tr1::function<Real (Real)> calc_grads) {
...
int main(int argc, const char *argv[]) {
  ...
  std::map<std::string, std::tr1::function<IActivation*()> > activation_factories;
